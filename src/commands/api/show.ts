...
export function registerApiShowCommand(api: Command) {
  api
    ...
    .action(async (ref: string, opts: OutputOptions & { org?: string }) => {
      ...
      try {
        // resolveApiId now hits PublicApisService.listTeamApis (V1); the
        // /apis/{apiId}/summary call below stays on the legacy client because
        // its rich shape (operations, env configs, subscriber count) isn't on
        // the V1 surface yet.
